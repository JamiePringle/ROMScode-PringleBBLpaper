...
# endif
      ...
      DO j=JstrP,JendT
         DO i=IstrT,IendT
            IF (tdays(ng).lt.0.5_r8) THEN
               windamp=tdays(ng)/0.5_r8*JMP_tauAmp
            ELSE IF (tdays(ng).lt.(0.5_r8+JMP_windDur)) THEN
               windamp=JMP_tauAmp
            ELSE
               windamp=max(0.0_r8,1.0_r8-                               &
     &              (tdays(ng)-0.5_r8-JMP_windDur)/0.5_r8)*             &
     &              JMP_tauAmp
            END IF
            if ( (yr(i,j).ge.(JMP_tau_y0-0.5*JMP_tau_yb)).and.          &
     &           (yr(i,j).le.(JMP_tau_y0+0.5*JMP_tau_yb))) then
               svstr(i,j)=windamp*                                      &
     &              cos(pi/JMP_tau_yb*(yr(i,j)-JMP_tau_y0))*            &
     &              0.5*(1.0_r8-tanh((xr(i,j)-JMP_tau_xoff)/            &
     &              JMP_tau_xwidth))
            else
               svstr(i,j)=0.0_r8 
            end if
